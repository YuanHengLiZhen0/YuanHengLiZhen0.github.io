...
- （2） 是否调用目标资源(即是否让用户访问web资源)。
    web服务器在调用doFilter方法时，会传递一个  filterChain对象进来，FilterChain对象是filter接口中最重要的一个对象，`它也提供了一个doFilter方法，开发人员可以根据需求决定是否调用此方法，调用该方法，则web服务器就会调用web资源的service方法`，即web资源就会被访问，否则web资源不会被访问。
- （3）调用目标资源之后，让一段代码执行。
